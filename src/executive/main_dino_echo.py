...
# Import own packagea
from src.data.dataloaders.video_data import EchoVideoDataset, none_or_int
from src.tools.preprocessing import DataAugmentationDINO


#augumentation
GaussianBlurT = lambda p, radius_min = 0.1, radius_max = 2: transforms.RandomApply([transforms.GaussianBlur(kernel_size = 3, sigma = (radius_min, radius_max))], p = p)
SolarizationT = lambda p, threshold = 0.5: transforms.RandomSolarize(threshold, p = p)
...
def get_args_parser():
    ...
    parser.add_argument('--img_size', default=224, type=int, help="The size to resize images to.")
    ...
    parser.add_argument('--data', default='EchoNet-Dynamics', type=str,
        help='Please specify dataloader type tu use.')
...
def train_dino(args):
    ...
    args.output_dir = os.path.join(args.output_dir, args.arch)
    ...
    if args.data == "EchoNet-Dynamics":
        ...
        dataset = EchoVideoDataset(root=args.data_path, split="train",
                                   length=args.length, transform=transform)
    else:
        # old data loading - imagenet
    ...
    lr_schedule = dino_utils.cosine_scheduler(
        args.lr * (args.batch_size_per_gpu * dino_utils.get_world_size()) / 256.,  # linear scaling rule
        args.min_lr,
